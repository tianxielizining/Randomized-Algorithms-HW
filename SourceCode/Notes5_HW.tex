...
Then give $n$ integers $x_1, x_2, ..., x_n$ in turn. 
What needs to be done is to select an integer from $x_1$ to $x_n$, 
and the goal is to make the value of the selected integer as large as possible. 
Since it is an online problem,
when  $x_i$ , $i\in\{1,2,...,n\}$ is given, the algorithm of the problem needs to immediately decide whether to choose $x_i$. 
Once an integer $x_i$ is chosen by an algorithm, the output of the algorithm of the problem is $x_i$.
Show that for any deterministic algorithm, for any constant $c\ge 1$,
there exists an input $x_1,x_2,...,x_n$, such that 
the maximum value of the input is at least $c$ times that of the output of the deterministic algorithm.
}\\
%Let $x_1,x_2,...,x_n$ be any instance of the online problem.
Let algorithm $A$ be any deterministic algorithm of the problem.
Let $x_1,x_2,...,x_n$ be any legal input of algorithm $A$.
Since algorithm $A$ is a deterministic algorithm and the problem is an online problem, 
at first when $x_1$ comes, 
algorithm $A$ need to decide whether to choose $x_1$ immediately only according to the value of $x_1$.
Once algorithm $A$ decides to choose $x_1$, $A$ must return $x_1$ and cannot choose any integer any more.
Once algorithm $A$ decides not to choose $x_1$, $A$ can not choose $x_1$ any more.
In a similar fashion, when $x_i$, $i\in \{1,2,...,n-1\}$ comes, algorithm $A$ decides whether to choose $x_i$ immediately according to $x_1,...,x_i$.
And when $x_n$ comes, which means that algorithm $A$ does not select any integer and have to choose and return $x_n$.
Assume that algorithm $A$ returns $x_a$, $a\in \{1,2,...,n\}$ at last.
We can construct an input so that the maximum value of the input is at least $c$ times that of the output of algorithm $A$ as follows.
If $a\in \{1,2,...,n-1\}$, let $x_1,x_2,...,x_a,cx_a,x_{a+2},...,x_n$ be the input.
If $a=n$, let $x_1,x_2,...,x_{n-1},cx_n$ be the input.
\\
