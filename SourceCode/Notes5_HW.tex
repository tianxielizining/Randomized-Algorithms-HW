...
it always submit the same $x_1$ to the black box.
Then algorithm $A$ use $f(x_1)$ to decide whether to access the black box for one more time.
If the answer is access to the black box, algorithm $A$ decide to which real number to be submitted according to $f(x_1)$.
In conclude, algorithm $A$ use $f(x_1), f(x_2),...,f(x_i)$ to decide whether $x_{i+1}$ exists and the value of $x_{i+1}$.
